...
//	// Compare shadow depth with current fragment depth
...
//	//return 0.0;
	...
	// Transform light fragment position from [-1, 1] to [0, 1] range
    //vec3 proj_light_frag_pos = light_frag_pos.xyz / light_frag_pos.w * 0.5 + 0.5;

    // Sample the depth map
    float shadow_depth = texture(shadowMap, projCoords.xy).r;

    // Compare fragment depth with shadow depth
    float bias = -0.000;  // Shadow bias to avoid self-shadowing
    float shadow = projCoords.z > shadow_depth ? 1.0 : 0.0;
		...
		for (int i = 0; i < MAX_LIGHTS; i++) {
			if (lights[i].enabled) {
				...
				if (normalMapMode == NORMAL_MAPPING_ON && objType == OBJTYPE_SPHERE) {
					...
					normal = normalize(texture(texCubeNorm, fragUV).rgb * 2.0 - 1.0);
				}
				else
				...
				//diffuse *= (1.0 - shadow);
				//specular *= (1.0 - shadow);
				outCol += (ambient + (diffuse + specular) * (1 - shadow)) * lights[i].color;  // TODO: use "shadow" variable here
				//outCol = vec3(shadow);
			}
		}
		outCol *= objColor;
